...
app.post('/api/addUser', (req, res) => {
  ...
  var hash_pwd = CryptoJS.SHA256(auth + auth_str + pwd).toString(CryptoJS.enc.Hex);

  db.run(`INSERT INTO user(username, firstname, lastname, email, password, auth) VALUES(?,?,?,?,?,?)`, [data.username, data.firstname, data.lastname, data.email, hash_pwd, auth_str], function(err) {
...
app.post('/api/loginUser', (req, res) => {
  ...
  db.all("SELECT auth FROM user WHERE username = ?", [data.username], (err, rows, result) => {
    if (err) {
      res.status(400).json({"error":err.message});
      return;
    }
    ...
    if(rows.length > 0){
      ...
      let pwd = data.password;
      let auth = "adb6f118edd6ca21cd88c2709b5b395266c0b3d71bd3c55bac875a31017c29fa";
      let hash_pwd = CryptoJS.SHA256(auth + auth_str + pwd).toString(CryptoJS.enc.Hex);

      let sql = "SELECT id, username FROM user WHERE username = ? and password = '"+ hash_pwd +"'";
